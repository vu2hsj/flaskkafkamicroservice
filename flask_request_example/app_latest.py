...
def insertDataDB(jsonType,jsonText):
    mydb = mysql.connector.connect(host="192.168.49.1",  user="testuser",  password="password",database="itemsdb")
    #mydb = mysql.connector.connect(host="localhost",  user="root",  password="root123",database="itemsdb")
...
@app.route('/sampleres')
def otherservicecall():
    response = requests.get("http://192.168.49.2:32101/test")
